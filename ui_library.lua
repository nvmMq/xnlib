...
Bracket.Assets = {
	...
	Window = function()
		...
		Background.TileSize = UDim2.new(0, 70, 0, 70)
	...
	KeybindList = function()
		...
		KeybindList.Size = UDim2.new(0, 180, 0, 90)
...
Bracket.Elements = {
	...
	Window = function(Window)
		...
		function Window.KeybindList(Self, KeybindList)
			...
			KeybindList.Size = Bracket.Utilities:GetType(KeybindList.Size, UDim2.new(0, 180, 0, 90), "UDim2")
			...
			Bracket.Utilities.MakeResizeable(Bracket.Screen.KeybindList.Resize, Bracket.Screen.KeybindList, Vector2.new(180, 90), Vector2.new(800, 800), function(Size)
	...
	Keybind = function(Parent, Window, Keybind)
		...
		Bracket.UInputBegan = UserInputService.InputBegan:Connect(function(Input, GameProcessedEvent)
			...
			if Keybind.Mouse then Key = Input.UserInputType.Name
				if Keybind.WaitingForBind and (Key == "MouseButton1" or Key == "MouseButton2" or Key == "MouseButton3") then
					Keybind.Value = Key
				elseif Key == "MouseButton1" or Key == "MouseButton2" or Key == "MouseButton3" then
					if Key == Keybind.Value then
		...
		Bracket.UInputEnded = UserInputService.InputEnded:Connect(function(Input, GameProcessedEvent)
			if GameProcessedEvent then return end
	...
	ToggleKeybind = function(Parent, Window, Keybind, Toggle)
		...
		Bracket.TUInputBegan = UserInputService.InputBegan:Connect(function(Input, GameProcessedEvent)
			if GameProcessedEvent then return end
		...
		Bracket.TUInputEnded = UserInputService.InputEnded:Connect(function(Input, GameProcessedEvent)
			if GameProcessedEvent then return end
		...
		Toggle:GetPropertyChangedSignal("Value"):Connect(function(Value)
			if Keybind.ListMimic then
				...
				pcall(function()
					Keybind.ListMimic.Asset.Tick.BackgroundColor3 = Value and Window.Color or Color3.fromRGB(63, 63, 63)
				end)
			end
		end)
	...
	Dropdown = function(Parent, Window, Dropdown)
		...
		local function RefreshSelected()
			...
			DropdownAsset.Background.Value.Text = #Dropdown.Internal.Value == 0 and "..." or table.concat(Dropdown.Internal.Value, ", ")
		end

		local function SetValue(Option, Value)
			Option.Value = Value
			Option.ColorConfig[1] = Value
			Option.Object.Tick.BackgroundColor3 = Value and Window.Color or Color3.fromRGB(63, 63, 63)
		end
		...
		local function AddOption(Option, AddToList, Order)
			...
			OptionAsset.Tick.BackgroundColor3 = Option.Value and Window.Color or Color3.fromRGB(63, 63, 63)
	...
	Colorpicker = function(Parent, Window, Colorpicker)
		...
		PaletteAsset.Rainbow.Tick.BackgroundColor3 = Colorpicker.Value[5] and Window.Color or Color3.fromRGB(63, 63, 63)
...
function Bracket:Window(Window)
	...
	function Window:Tab(Tab)
		...
		function Tab:AddConfigSection(FolderName)
			local ConfigSection = Tab:Section({ Name = "Configs" }) do
				...
				local ConfigDivider = ConfigSection:Divider({Text = not ALConfig and "AutoLoad Config" or "AutoLoad Config\n<font color=\"rgb(189, 189, 189)\">[ " .. ALConfig .. " ]</font>"})
		...
		function Tab:Toggle(Toggle)
			...
			function Toggle:Keybind(Keybind)
				...
				Keybind.Blacklist = Bracket.Utilities:GetType(Keybind.Blacklist, {"W", "A", "S", "D", "Slash", "Tab", "Backspace", "Escape", "Space", "Unknown", "Backquote"}, "table")
...
function Bracket:Push(Notification)
	...
	NotificationAsset.Size = UDim2.fromOffset((NotificationAsset.Title.TextBounds.X > NotificationAsset.Description.TextBounds.X and NotificationAsset.Title.TextBounds.X or NotificationAsset.Description.TextBounds.X) + 24, NotificationAsset.ListLayout.AbsoluteContentSize.Y + 8)
...
function Bracket:Unload(Callback)
	...
	Bracket.Elements = nil

	Bracket.Utilities = nil

	pcall(function()
		Bracket.UInputBegan:Disconnect()

		Bracket.TUInputBegan:Disconnect()

		Bracket.UInputEnded:Disconnect()

		Bracket.TUInputEnded:Disconnect()
	end)
