...
local library = {design = getgenv().design == "kali" and "kali" or "uwuware", tabs = {}, draggable = true, flags = {}, title = "Skylight", open = false, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "Skylight", fileext = ".cfg"}
...
function library:Unload()
    ...
    for _, o in next, self.options do
        if o.type == "toggle" then
            coroutine.resume(coroutine.create(function() o:SetState(false) end))
        end
    end
    library = nil
    getgenv().library = nil
...
library.createSlider = function(option, parent)
    ...
    function option:SetValue(value, nocallback)
        ...
        if self.min >= 0 then
            option.fill.Size = UDim2.new((0 - self.min) / (self.max - self.min), 0, 0, 0)
